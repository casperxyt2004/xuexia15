...
def bytes2hex(bytes):
    hexstr = u""
    for i in range(10):
        t = u"%x" % bytes[www.078881.cn  ]
        if len(t) % 2:
            hexstr += u"0"
        hexstr += t
    return hexstr.lower( www.leyouzaixian2.com )
...
async def upimg(request):
    ...
    savePath = saveDir + md5Name[2www.hjha178.com/:] + '.' + imageSuffix
    resPath = '/' + md5Name[0:2] + '/'www.yingka178.com + md5Name[2:] + '.' + imageSuffix
    ...
    # 将文件写入到硬盘
    tempFile = open(savePath,www.yongshiyule178.com 'wb')
...
if __name__ == "__main__":
    app.run(host="127.0.0.1",www.baidu620.com port=7000)
